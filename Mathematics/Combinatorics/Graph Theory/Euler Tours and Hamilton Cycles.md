...
> [!tips] **Theorem (Dirac's Theorem)**
> If a simple graph $G$ with $n \ge 3$ vertices has a minimum degree of $\delta(G) \ge n/2$, then $G$ is Hamiltonian.
>
...
> [!tips] **Theorem (Ore's Theorem)**
> If a simple graph $G$ with $n \ge 3$ vertices satisfies $\deg(u) + \deg(v) \ge n$ for every pair of non-adjacent vertices $u$ and $v$, then $G$ is Hamiltonian.
>
...
> [!tips] **Theorem (Hall's Marriage Theorem)**
> In a bipartite graph $G = (U, V, E)$, a matching that covers every vertex in $U$ exists if and only if for every subset $S \subseteq U$, the size of its neighborhood is at least the size of the subset itself ($|N(S)| \ge |S|$).
>
> **Proof Idea:** If there is a subset of vertices in $U$ that collectively connect to fewer vertices in $V$ (a "bottleneck"), it's impossible to match them all. Hall's theorem proves the more difficult reverse direction: if no such bottleneck exists, a complete matching is always possible.
>
> **Usefulness:** The fundamental theorem for solving assignment and allocation problems in bipartite graphs.

> [!tips] **Theorem (Kőnig's Theorem)**
